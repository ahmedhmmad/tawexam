...
class MockLoadExamSessionUseCase extends Mock implements LoadExamSessionUseCase {}
class MockLoadQuestionsUseCase extends Mock implements LoadQuestionsUseCase {}
class MockRestoreExamProgressUseCase extends Mock implements RestoreExamProgressUseCase {}
class MockSaveAnswerUseCase extends Mock implements SaveAnswerUseCase {}
class MockSaveFlaggedQuestionsUseCase extends Mock implements SaveFlaggedQuestionsUseCase {}
...
void main() {
  ...
  const student = Student(id: 's1', seatNumber: 'S001', fullName: 'Ali', branch: 'A', schoolName: 'Sch');
  ...
  const exam = Exam(
    id: 'e1', subjectNameArabic: 'رياضيات', subjectNameEnglish: 'Math',
    duration: Duration(hours: 1), totalQuestions: 2, instructions: [],
  );
  const questions = [
    Question(id: 'q1', text: 'Q1', options: [], order: 1),
    Question(id: 'q2', text: 'Q2', options: [], order: 2),
  ];
  const progress = RestoredExamProgress(answers: {}, flaggedQuestionIds: {});
  const result = ExamResult(examId: 'e1', score: 80, totalQuestions: 2, correctAnswers: 1, items: []);

  setUpAll(() {
    registerFallbackValue(const SaveAnswerParams(sessionId: '', examId: '', questionId: '', answerId: ''));
    registerFallbackValue(SubmitExamParams(sessionId: '', examId: '', answers: const {}, submittedAt: DateTime(2026)));
  });
  ...
  void stubSuccess() {
    when(() => loadExam()).thenAnswer((_) async => const Right(exam));
    when(() => loadSession(any())).thenAnswer((_) async => Right(session));
    when(() => loadQuestions(any())).thenAnswer((_) async => const Right(questions));
    when(() => restoreProgress(any())).thenAnswer((_) async => const Right(progress));
  }

  blocTest<ExamCubit, ExamState>(
    'emits ExamLoading then ExamReady on loadForStudent success',
    build: () { stubSuccess(); return buildCubit(); },
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    build: () {
      when(() => loadExam()).thenAnswer((_) async => const Left(ServerFailure('err')));
      return buildCubit();
    },
    act: (c) => c.loadForStudent(student: student),
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    build: () { stubSuccess(); return buildCubit(); },
    seed: () => ExamReady(
      student: student, session: session, exam: exam, questions: questions,
      answers: const {}, flagged: const {}, currentIndex: 0,
      remaining: const Duration(hours: 1), syncStatus: SyncStatus.idle,
    ),
    act: (c) {
      when(() => saveAnswer(any())).thenAnswer((_) async => const Right(unit));
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    act: (c) {
      when(() => saveFlagged(sessionId: any(named: 'sessionId'), flaggedQuestionIds: any(named: 'flaggedQuestionIds')))
          .thenAnswer((_) async => const Right(unit));
      return c.toggleFlag();
    },
    expect: () => [isA<ExamReady>().having((s) => s.flagged, 'flagged', contains('q1'))],
  );
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    expect: () => [isA<ExamReady>().having((s) => s.currentIndex, 'index', 1)],
  );
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    act: (c) {
      ...
      when(() => submitExam(any())).thenAnswer((_) async => const Right(result));
      return c.submitExam();
    },
  ...
  blocTest<ExamCubit, ExamState>(
    ...
    seed: () => ExamReady(
      student: student, session: session, exam: exam, questions: questions,
      answers: const {'q1': 'a'}, flagged: const {'q1'}, currentIndex: 1,
      remaining: const Duration(hours: 1), syncStatus: SyncStatus.synced,
    ),
    act: (c) => c.retryExam(),
    expect: () => [
      isA<ExamReady>()
