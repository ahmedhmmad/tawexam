...
export class ResultsController {
  async studentResult(req: Request, res: Response): Promise<Response> {
    const result = await resultsService.getStudentResult(req.params.id, req.user!.id);
    return sendSuccess(res, result);
  }

  async analytics(req: Request, res: Response): Promise<Response> {
    const result = await resultsService.analytics(req.params.id);
    return sendSuccess(res, result);
  }

  async export(req: Request, res: Response): Promise<Response> {
    const buffer = await resultsService.exportExamResults(req.params.id);
